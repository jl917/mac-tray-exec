...
| `keepAwake` | 앱이 떠 있는 동안 화면 잠금·슬립 방지 (기본값 `true`). 끄려면 `false` |

### 화면 잠금 방지

앱이 실행 중인 동안 디스플레이가 꺼지거나 화면이 잠기지 않습니다.
내부적으로 macOS 기본 `caffeinate -d -i`를 자식 프로세스로 띄워 전원 어서션을 유지합니다.

- 메뉴바 → **화면 잠금 방지: 켜짐 ✓** 항목으로 실행 중에 끄고 켤 수 있습니다
  (이렇게 직접 바꾼 상태는 "메뉴 새로 고침"으로 되돌아가지 않습니다).
- 항상 꺼진 상태로 시작하려면 설정에 `"keepAwake": false`를 넣습니다.
- 앱이 종료되면(크래시·강제 종료 포함) 어서션도 함께 풀립니다 — `caffeinate -w <앱 PID>`로 물려 두기 때문입니다.
- 확인: `pmset -g assertions | grep caffeinate`
- 노트북 덮개를 닫는 것까지 막지는 않습니다.
...
메뉴 **첫 줄에는 항상 현재 버전**(`mac-tray-exec 2.0.1`)이 표시됩니다. 표시 전용이라 눌러도 아무 일도 하지 않습니다.
이 값은 빌드할 때 `package.json`에서 가져오므로 따로 관리할 필요가 없습니다.

사용자 항목 아래에는 **설정 파일 편집… / 로그 보기 / 메뉴 새로 고침 / 종료**가 자동으로 붙습니다.
